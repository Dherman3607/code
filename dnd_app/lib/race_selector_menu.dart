...
class RaceSelectorMenu extends StatefulWidget {
  ...
  const RaceSelectorMenu(
      {Key? key, required this.races, required this.onSelected})
      : super(key: key);
                ...
                children: [
                  ...
                  if (r.traits.isNotEmpty) ...[
                    ...
                    ...r.traits.entries.expand<Widget>((e) {
                      ...
                      final value = e.value;
                      String formatted;
                      // Type-aware formatting for common numeric/range traits
                      if (value is List) {
                        formatted = value.map((v) => v.toString()).join(', ');
                      } else if (label.toLowerCase() == 'speed') {
                        formatted = TraitFormatter.formatDistance(value) ?? '';
                      } else if (label.toLowerCase() == 'darkvision') {
                        formatted = TraitFormatter.formatDistance(value) ?? '';
                      } else if (label.toLowerCase() == 'hitdice' ||
                          label.toLowerCase() == 'hit_die') {
