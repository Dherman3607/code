  ...
  void _toggleExpand(int index) {
    ...
    if (_expandedIndex != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        if (ctx != null) Scrollable.ensureVisible(ctx, duration: const Duration(milliseconds: 360), alignment: 0.02);
      });
    }
  }
      ...
      child: Column(
        ...
        children: [
          ListTile(
            ...
            subtitle: r.traits['size'] != null ? Text('Size: ${r.traits['size']}', style: const TextStyle(color: Colors.white70)) : null,
          ...
          if (expanded)
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  if (r.description.isNotEmpty) ...[Text(r.description, style: const TextStyle(color: Colors.white70)), const SizedBox(height: 8)],
                  if (r.traits.isNotEmpty) ...[
                    const Text('Traits:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
                    ...
                    ...r.traits.entries.expand<Widget>((e) {
                      ...
                      return [
                        Text('${label[0].toUpperCase()}${label.substring(1)}:', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.end, children: [ElevatedButton(onPressed: () => widget.onSelected(r), style: ElevatedButton.styleFrom(backgroundColor: Colors.blueGrey[700], foregroundColor: Colors.white), child: const Text('Select'))])
                ],
              ),
            )
        ],
  ...
  Widget build(BuildContext context) {
    return Dialog(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFF3B4852), Color(0xFF2E3538), Color(0xFF242627)], stops: [0.0, 0.6, 1.0]),
          borderRadius: BorderRadius.circular(12),
        ),
        child: Column(children: [
          Padding(padding: const EdgeInsets.all(12), child: Row(children: [const Expanded(child: Text('Select a Race', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white))), IconButton(icon: const Icon(Icons.close, color: Colors.white), onPressed: () => Navigator.of(context).pop())])),
          const Divider(height: 1, color: Colors.white24),
          Expanded(child: ListView.builder(padding: const EdgeInsets.symmetric(vertical: 8), itemCount: widget.races.length, itemBuilder: (context, index) {
